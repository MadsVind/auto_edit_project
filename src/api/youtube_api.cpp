...
void YouTubeApi::uploadVideo(const std::string& video_path, 
    ...
    if (!file.good()) {
        throw std::runtime_error("Can't open the video file: " + video_path);
    }
    ...
    body_json["status"]["madeForKids"] = false;
...
bool YouTubeApi::isTokenValid() {
    cpr::Response r = cpr::Get(cpr::Url{"https://www.googleapis.com/oauth2/v1/tokeninfo"},
                               cpr::Parameters{{"access_token", getAccessToken()}});
    if (r.status_code != 200) std::cout << r.text << std::endl;
